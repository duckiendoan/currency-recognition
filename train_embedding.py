...
from models import ViTSupCon, ResNet50SupCon
...
if __name__ == '__main__':
    ...
    labels = list(itertools.chain(*(dataset['labels'] for dataset in datasets)))
    ...
    data_module = CustomDataModule(train_df, valid_df, labels, TwoCropTransformAlbumentation(train_transform),
                                   val_transform, batch_size=BATCH_SIZE)
    if args.model == 'resnet50':
        model = ResNet50SupCon(learning_rate=LEARNING_RATE)
    else:
        model = ViTSupCon(model_name=MODEL_NAME, learning_rate=LEARNING_RATE)
